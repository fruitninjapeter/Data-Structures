class Queue:
    ...
    def enqueue(self, item):
        ...
        self.rear = (self.rear + 1) % self.capacity  # increment the rear pointer
        self.num_in_queue += 1

    def dequeue(self):
        if self.is_empty():  # if queue is empty.txt raise IndexError
            raise IndexError
        data = self.values[self.front]
        self.values[self.front] = None
        self.front = (self.front + 1) % self.capacity  # increment the front pointer
